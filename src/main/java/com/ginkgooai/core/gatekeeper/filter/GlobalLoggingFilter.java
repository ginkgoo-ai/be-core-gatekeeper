package com.ginkgooai.core.gatekeeper.filter;
...
public class GlobalLoggingFilter extends OncePerRequestFilter {
	...
	private static final List<String> JSON_CONTENT_TYPES = Arrays.asList("application/json",
			"application/json;charset=UTF-8", "application/json;charset=utf-8");

	private static final List<String> EXCLUDE_PATHS = Arrays.asList("/actuator", "/swagger", "/v3/api-docs",
			"/favicon.ico", "/static", "/webjars");
	...
	private void logApiCall(ContentCachingRequestWrapper request, ContentCachingResponseWrapper response,
		...
		try {
			...
			log.info("API Call - {} {} - Status: {} - Time: {}ms\n-> Request: {}\n<- Response: {}", request.getMethod(),
					getFullRequestPath(request), response.getStatus(), timeElapsed, requestBody, responseBody);
		}
		catch (Exception e) {
	...
	private boolean isJsonContent(String contentType) {
		...
		return JSON_CONTENT_TYPES.stream().anyMatch(lowerContentType::contains);
	}
